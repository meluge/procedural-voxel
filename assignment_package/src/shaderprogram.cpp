...
void ShaderProgram::drawWeaved(Drawable &d, BufferType index, BufferType transparency) {
    if (d.elemCount(index)< 0) {
        throw std::invalid_argument("not initialized");
    }

    int handle;
    if ((handle = m_attribs["vs_Pos"]) != -1 && d.bindBuffer(transparency)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), nullptr);
        // context->glVertexAttribDivisor(handle, 0);
    }
    if ((handle = m_attribs["vs_Nor"]) != -1 && d.bindBuffer(transparency)) {
        context->glEnableVertexAttribArray(handle);
        context->glVertexAttribPointer(handle, 4, GL_FLOAT, false, 3 * sizeof(glm::vec4), (void*)sizeof(glm::vec4));
    }
    if ((handle = m_attribs["vs_UV"]) != -1 && d.bindBuffer(transparency)) {
        context->glEnableVertexAttribArray(handle);
...
void ShaderProgram::drawRain(Drawable &d) {

    useMe();

    glEnableVertexAttribArray(m_attribs["vs_Pos"]);
    d.bindBuffer(POSITION);
    glVertexAttribPointer(m_attribs["vs_Pos"], 4, GL_FLOAT, GL_FALSE, 0, nullptr);

    glEnableVertexAttribArray(m_attribs["vs_ID"]);
    d.bindBuffer(RAINID);
    glVertexAttribIPointer(m_attribs["vs_ID"], 1, GL_INT, 0, nullptr);

    d.bindBuffer(INDEX);

    glDrawElements(GL_TRIANGLES, d.elemCount(INDEX), GL_UNSIGNED_INT, nullptr);

    glDisableVertexAttribArray(m_attribs["vs_Pos"]);
    glDisableVertexAttribArray(m_attribs["vs_ID"]);
    // if(d.elemCount(INDEX) < 0) {
    //     throw std::invalid_argument(
    //         "Attempting to draw a Drawable that has not initialized its count variable! Remember to set it to the length of your index array in create()."
    //         );
    ...
    // if ((handle = m_attribs["vs_ID"]) != -1 && d.bindBuffer(RAINID)) {
    //
    //     context->glEnableVertexAttribArray(handle);
    //     context->glVertexAttribIPointer(handle, 1, GL_INT, 0, nullptr);
    // }
    ...
    // if (m_attribs["vs_Pos"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_Pos"]);
    // if (m_attribs["vs_ID"] != -1) context->glDisableVertexAttribArray(m_attribs["vs_ID"]);
