...
InventoryRectangle::InventoryRectangle(OpenGLContext* context) : Drawable(context) {}

InventoryRectangle::~InventoryRectangle() {}

void InventoryRectangle::createVBOdata() {
    std::vector<glm::vec4> positions {
        glm::vec4(-1.f, -1.f, 0.f, 1.f),  // Bottom left
        glm::vec4( 1.f, -1.f, 0.f, 1.f),  // Bottom right
        glm::vec4( 1.f,  1.f, 0.f, 1.f),  // Top right
...
void InventoryRectangle::initializeAndBufferGeometryData() {
    createVBOdata();
}

void InventoryRectangle::updateUVs(const std::vector<glm::vec2>& newUVs) {
    uvs = newUVs;
    bindBuffer(UV);
    mp_context->glBufferData(GL_ARRAY_BUFFER, uvs.size() * sizeof(glm::vec2), uvs.data(), GL_STATIC_DRAW);
}
