...
Entity::Entity(glm::vec3 pos)
    : m_forward(0,-1,-1), m_right(1,0,0), m_up(0,1,0), m_position(pos), mcr_position(m_position),  prev_pos(m_position + glm::vec3(1000.0f)), flightMode(true), isOnGround(true)
{}

Entity::Entity(const Entity &e)
    : m_forward(e.m_forward), m_right(e.m_right), m_up(e.m_up), m_position(e.m_position), mcr_position(m_position), prev_pos(m_position + glm::vec3(1000.0f)), flightMode(e.flightMode), isOnGround(e.isOnGround)
{}
...
glm::vec3 Entity::getF(){
    return m_forward;
}

glm::vec3 Entity::getR(){
    return m_right;
}

glm::vec3 Entity::getU(){
    return m_up;
}
